      ...
      for (const p of sites) {
        ...
        cy.get('a').each(page => {
          ...
          if (page.prop('href').startsWith('http')) {
            // Three websites seem to be inconsistent with their 404 pages, so we'll just skip them for now.'
            if (!page.contents().text().includes('Xiaoying Riley')
              && !page.prop('href').includes('bhp.com')
              && !page.prop('href').includes('element14.com'))
            {
